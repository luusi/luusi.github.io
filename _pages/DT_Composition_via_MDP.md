...
$P_z(<b>{\sigma'}</b> \mid <b>{\sigma}</b>, (a, i)) = P(\sigma_i' \mid \sigma_i, a)$, for $<b>{\sigma}</b> = (\sigma_1\dots\sigma_n)$,
$<b>{\sigma'}</b> = (\sigma_1'\dots\sigma_n')$ and $a\in A_i(\sigma_i)$, with $\sigma_i\in\Sigma_i$ and  $\sigma_j = \sigma'_j$ for $j\neq i$, $R_z(<b>{\sigma}</b>, (a, i)) = R_i(\sigma_i, a)$ for $<b>{\sigma}</b>\in \Sigma_z$, $a\in A_i(\sigma_i)$.
